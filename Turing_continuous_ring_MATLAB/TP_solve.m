...
function [sol] = TP_solve(P,tmax,n,L)
    ...
    TPfunctions = TPfuns; %here we call the necessary functions to solve the PDE:
    ...
    %%PDEPE solver
    sol = pdepe(0,@TPfunctions.fun3,@TPfunctions.fun1,@TPfunctions.fun2,x,t,[],P);
    ...
    axis([0 L 0 tmax min(min(u1)) max(max(u1))])
    ...
    axis([0 L 0 tmax min(min(u2)) max(max(u2))])
    set(gcf(), 'Renderer', 'painters')
    set(gca,'FontSize',18,'fontweight','b','fontname','arial')
    ...
    for n = 1:length(t)
        ...
        axis([0 L  min(min(min(sol(:,:,:))))-0.1 max(max(max(sol(:,:,:))))+0.1])
        M(n) = getframe;
    end
